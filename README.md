...
| 版本 | 文件名 | 函数名 | 特点 |
|------|--------|--------|------|
| 🌟 **英文版** | `SamplingCalculation.vba` | `GetSampleSize()` | 无乱码，兼容性好 |
| 中文版 | `抽样计算_改进版.vba` | `获取样本量()` | 更直观，但可能乱码 |

**推荐使用英文版** ✅

1. 菜单：插入 → 模块
2. 打开 `SamplingCalculation.vba`
3. 复制全部代码粘贴到模块窗口
4. 关闭VBA编辑器（按 `Alt + Q`）
...
✅ 完成！详细步骤和问题排查见 [如何重建Excel文件.md](如何重建Excel文件.md)
...
| 函数名 | 说明 | 示例 | 返回值 |
|--------|------|------|--------|
| `GetSampleSize()` | 计算抽样样本量 | `=GetSampleSize(150,"II",1.5)` | 20 |
| `GetAcValue()` | 获取接收数 | `=GetAcValue(150,"II",1.5)` | 1 |
| `GetReValue()` | 获取拒收数 | `=GetReValue(150,"II",1.5)` | 2 |
| `GetInspectionType()` | 判断全检/抽检 | `=GetInspectionType(150,"II",1.5)` | Sampling Inspection |
